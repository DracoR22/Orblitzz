                ...
                {item.name === "Pro" && (
                 ...
                 <div className="p-5">
                    ...
                    {currentPlan && currentPlan.name === item.name && (
                      <Button onClick={() => onClick(item.price.priceIds.production, i)} variant={'default'} className="w-full  mt-4 text-white">
                       {loadingIndex === i ? <Loader2Icon className="w-5 h-4 animate-spin"/> : `Manage Plan`}
                     </Button>
                    )}

                    {isDashboard && currentPlan && currentPlan.name !== item.name && (
                        <Button onClick={() => onClick(item.price.priceIds.production, i)} variant={'outline'} className="w-full dark:bg-[#242424] bg-[#fafafa] mt-4 dark:hover:bg-[#1e1e1e] hover:bg-white">
